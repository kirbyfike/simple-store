...
function Coffers() {
  ...
  if(product.variants.length) {
    ...
    typeOptions1 = product.variants.map(c => <option key={c.main.id} value={c.main.sku}>{c.main.sku}</option>);
  }
  ...
  if (selectedVariant) {
    console.log("hereee", selectedVariant);
    variantInfo = (
        <div className="pt-4">
          ...
          <div className="flex items-baseline mb-1 space-x-2 font-roboto mt-4">
            ...
            {selectedVariant.addOns.map((row, index) => (
                <div style={{border: '1px solid', margin: '3px', padding: '3px', width: '550px', textAlign: 'center'}} key={index}>
                    <h4 style={{backgroundColor: '#efefef'}}>{row.name['en-US']} - ${row.masterVariant.prices[0].value.centAmount / 100}</h4>
                    <br />
                    <button
