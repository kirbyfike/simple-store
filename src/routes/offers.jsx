...
function Offers() {
  ...
  const fetchProduct = async (productId) => {
    ...
    if(res && res.body) {
        let addOns = await fetchAddons(res.body.masterVariant.attributes);
        let myOffer = {
            offer: res.body,
            addOns: addOns
        }
  ...
  const fetchAddons = async (attributes) => {
    ...
    for (const [key, value] of Object.entries(attributes)) {
        console.log(value.name === "products")
        
        if (value.name === "products") {
            formObject.main =  await fetchAPIProduct(value.value[0].id);
        }

        if (value.name === "includedOffers") {
  ...
  const onChangeType1 = (event) => {
    console.log("my var 1", variantName);
    //setVariantName(event.target.value);

    console.log("my var", variantName);
  }
  ...
  if(product.offer.variants.length) {
    let myVars = [product.offer.masterVariant];
    myVars.push(...product.offer.variants);
    
    typeOptions1 = myVars.map(c => <option key={c.id} value={c.sku}>{c.sku}</option>);
  }

  let typeOptions2 = "";
  if([].length) {
    typeOptions1 = product.offer.variants.map(c => <option key={c.id} value={c.sku}>{c.sku}</option>);
  }
        ...
        <div className="container grid grid-cols-2 gap-6">
          ...
          <div>
            <h2 className="text-3xl font-medium uppercase mb-2">
              {product.offer.name[config.locale]}
            </h2>
            ...
            <div className="pt-4">
                ...
                {product.addOns.sub.map((row, index) => (
                    <h4 key={index}>{row.name.en}</h4>
                ))}
            </div>
